...
class ScamShieldPredictor:
    def __init__(self, model_path: str | None = None):
        ...
        path = Path(model_path) if model_path else (base.parent / "models" / "model.pkl")
        if not path.exists():
    ...
    def analyze(self, url: str) -> dict:
        ...
        x = pd.DataFrame([{name: features.get(name, 0.0) for name in self.feature_names}], columns=self.feature_names)
